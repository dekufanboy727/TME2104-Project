...
function ValidateName(name) //Validate first and last name
{
    ...
    else //Check whether the other characters are in lower case?
    {
        for(x=1; x<name.length; x++)
        {
            ...
            {
                alert("Please make sure the other characters after the first character are all in lowercase for Name!")
                y = 0;
                break;
            }
            else
        ...
        if(y == 0) //If there is uppercase letter after the first character
        {
            return false;
        }
    }
